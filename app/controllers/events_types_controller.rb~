class EventsTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @events_type.save
        format.html { redirect_to(@events_type, :notice => 'Events type was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @events_type.update_attributes(params[:events_type])
        format.html { redirect_to(@events_type, :notice => 'Events type was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(events_types_url) }
      format.xml  { head :ok }
    end
  end
end
